...
app.put("/books/:id", async (req, res) => {
  const bookId = req.params.id; // Get book id from URL parameter
  ...
  try {
    ...
    const updatedBookResult =
      await getUpdatedBookRequest.query(getUpdatedBookQuery);

    if (updatedBookResult.recordset.length === 0) {
      return res.status(404).send("Book not found");
    }
...
app.delete("/books/:id", async (req, res) => {
  ...
  try {
    ...
    const checkBookQuery = `SELECT id FROM Books WHERE id = @id`;
    const checkRequest = connection.request();
    checkRequest.input("id", bookId);
    const checkResult = await checkRequest.query(checkBookQuery);
    ...
    res.status(200).send("Book deleted successfully"); // Send 200 OK status
  } catch (error) {
